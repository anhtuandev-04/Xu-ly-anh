...
{
        QImage image_in(ui->ln_filename->text());
        QImage image_out_Gx(image_in.width()-1, image_in.height()-1, QImage::Format_ARGB32);
        QImage image_out_Gy(image_in.width()-1, image_in.height()-1, QImage::Format_ARGB32);
        for (int i = 0; i <image_in.width()-1; i++)
        {
            for (int j = 0; j <image_in.height ()-1; j++)
            {
                ...
                int newgray_qx = abs (gray1 - gray4);
                int newgray_qy = abs (gray2 - gray4);
                image_out_Gx.setPixel (1, j, qRgb (newgray_qx, newgray_qx, newgray_qx));
                image_out_Gy.setPixel (i, j, qRgb (newgray_qy, newgray_qy, newgray_qy));
            }
        }
        QLabel *label_in = new QLabel();
        label_in->setPixmap(QPixmap::fromImage(image_in.scaled (1024,768)));
...
void MainWindow::on_btn_gauss_laplape_clicked()
{
    QImage image_in(ui->ln_filename->text());
        ...
        // Ma trận Gauss kết hợp Laplace (5x5) từ tài liệu
        int maskSize = 5;
        int margin = maskSize / 2;
        int mask[5][5] = {
            {0,  0, -1,  0,  0},
    ...
    QImage image_in(ui->ln_filename->text());
        ...
        for (int i = 0; i < image_in.width() - 1; i++)
        {
            for (int j = 0; j < image_in.height(); j++)
            {
                ...
                int r1 = qRed(color1), g1 = qGreen(color1), b1 = qBlue(color1);
                int r2 = qRed(color2), g2 = qGreen(color2), b2 = qBlue(color2);

                int newR = abs(r1 - r2);
                int newG = abs(g1 - g2);
                int newB = abs(b1 - b2);

                image_out_Gx.setPixel(i, j, qRgb(newR, newG, newB));
            }
        }
        ...
        for (int i = 0; i < image_in.width(); i++)
        {
            for (int j = 0; j < image_in.height() - 1; j++)
            {
                ...
                image_out_Gy.setPixel(i, j, qRgb(newR, newG, newB));
            }
        }
    ...
    QImage image_out_Gx(image_in.width() - 1, image_in.height() - 1, QImage::Format_RGB32);
        ...
        for (int i = 0; i < image_in.width() - 1; i++)
        {
            for (int j = 0; j < image_in.height() - 1; j++)
            {
                ...
                int r1 = qRed(color1), g1 = qGreen(color1), b1 = qBlue(color1);
                int r2 = qRed(color2), g2 = qGreen(color2), b2 = qBlue(color2);
                int r3 = qRed(color3), g3 = qGreen(color3), b3 = qBlue(color3);
                int r4 = qRed(color4), g4 = qGreen(color4), b4 = qBlue(color4);

                int newR_Gx = abs(r1 - r4);
                int newG_Gx = abs(g1 - g4);
                int newB_Gx = abs(b1 - b4);

                int newR_Gy = abs(r2 - r3);
                int newG_Gy = abs(g2 - g3);
                int newB_Gy = abs(b2 - b3);

                image_out_Gx.setPixel(i, j, qRgb(newR_Gx, newG_Gx, newB_Gx));
                image_out_Gy.setPixel(i, j, qRgb(newR_Gy, newG_Gy, newB_Gy));
            }
        }
...
{
    QImage image_in(ui->ln_filename->text()); // Đầu vào ảnh từ ln_filename->text()
            ...
            for (int x = margin; x < image_in.width() - margin; x++)
            {
                ...
                for (int j = -margin; j <= margin; j++)
                {
                    for (int i = -margin; i <= margin; i++)
                    {
                        ...
                        sumR += r * maskX[j + margin][i + margin];
                        sumG += g * maskX[j + margin][i + margin];
                        sumB += b * maskX[j + margin][i + margin];

                        sumR += r * maskY[j + margin][i + margin];
                        sumG += g * maskY[j + margin][i + margin];
                        sumB += b * maskY[j + margin][i + margin];
                    }
                }

                // Tính độ lớn Gradient cho mỗi kênh màu (có thể dùng căn bậc hai, nhưng đơn giản hóa bằng abs)
                int gradR = qBound(0, abs(sumR), 255);
                int gradG = qBound(0, abs(sumG), 255);
                int gradB = qBound(0, abs(sumB), 255);

                image_out_Gx.setPixel(x, y, qRgb(gradR, gradG, gradB));
                image_out_Gy.setPixel(x, y, qRgb(gradR, gradG, gradB)); // Có thể tách Gx và Gy riêng nếu cần
            }
        }
...
void MainWindow::on_btn_loc_prewitt_anhmau_clicked()
{
    QImage image_in(ui->ln_filename->text());
        ...
        QRgb color;
        int sumR_Gx, sumG_Gx, sumB_Gx;
        int sumR_Gy, sumG_Gy, sumB_Gy;

        for (int y = margin; y < image_in.height() - margin; y++)
        {
            for (int x = margin; x < image_in.width() - margin; x++)
            {
                sumR_Gx = sumG_Gx = sumB_Gx = 0;
                sumR_Gy = sumG_Gy = sumB_Gy = 0;
                ...
                for (int j = -margin; j <= margin; j++)
                {
                    for (int i = -margin; i <= margin; i++)
                    {
                        ...
                        // Tính Gradient cho hướng X
                        sumR_Gx += r * maskX[j + margin][i + margin];
                        sumG_Gx += g * maskX[j + margin][i + margin];
                        sumB_Gx += b * maskX[j + margin][i + margin];

                        // Tính Gradient cho hướng Y
                        sumR_Gy += r * maskY[j + margin][i + margin];
                        sumG_Gy += g * maskY[j + margin][i + margin];
                        sumB_Gy += b * maskY[j + margin][i + margin];
                    }
                }

                // Giới hạn giá trị Gradient trong khoảng 0-255
                int gradR_Gx = qBound(0, abs(sumR_Gx), 255);
                int gradG_Gx = qBound(0, abs(sumG_Gx), 255);
                int gradB_Gx = qBound(0, abs(sumB_Gx), 255);

                int gradR_Gy = qBound(0, abs(sumR_Gy), 255);
                int gradG_Gy = qBound(0, abs(sumG_Gy), 255);
                int gradB_Gy = qBound(0, abs(sumB_Gy), 255);

                image_out_Gx.setPixel(x, y, qRgb(gradR_Gx, gradG_Gx, gradB_Gx));
                image_out_Gy.setPixel(x, y, qRgb(gradR_Gy, gradG_Gy, gradB_Gy));
            }
        }
...
void MainWindow::on_btn_loc_laplace_anhmau_clicked()
{
    QImage image_in(ui->ln_filename->text());
        ...
        // Ma trận Laplace (dạng đơn giản 3x3) từ tài liệu
        int mask[3][3] = {
            {0,  1,  0},
            {1, -4,  1},
            {0,  1,  0}
            ...
            for (int x = margin; x < image_in.width() - margin; x++)
            {
                ...
                for (int j = -margin; j <= margin; j++)
                {
                    for (int i = -margin; i <= margin; i++)
                    {
                        color = image_in.pixel(x + i, y + j);
                        int r = qRed(color), g = qGreen(color), b = qBlue(color);
                        sumR += r * mask[j + margin][i + margin];
                        sumG += g * mask[j + margin][i + margin];
                        sumB += b * mask[j + margin][i + margin];
                    }
                }

                // Giới hạn giá trị Laplace trong khoảng 0-255
                int lapR = qBound(0, sumR, 255);
                int lapG = qBound(0, sumG, 255);
                int lapB = qBound(0, sumB, 255);

                image_out.setPixel(x, y, qRgb(lapR, lapG, lapB));
            }
        }
...
void MainWindow::on_btn_themnhieu_anhmau_clicked()
{
    QImage image_in(ui->ln_filename->text()); // Đầu vào ảnh từ LineFileName
        QImage image_out(image_in.width(), image_in.height(), QImage::Format_RGB32);

        double noiseStrength = 0.2; // Mức độ nhiễu (tỷ lệ nhân, từ 0 đến 1, 0.2 nghĩa là 20% nhiễu)

        // Khởi tạo seed cho số ngẫu nhiên dựa trên thời gian hiện tại
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> dis(0.0, 1.0); // Phân phối đồng nhất từ 0 đến 1
        ...
        for (int y = 0; y < image_in.height(); y++)
        {
            for (int x = 0; x < image_in.width(); x++)
            {
                ...
                // Tạo hệ số nhiễu Speckle
                double speckleFactor = 1.0 + (dis(gen) - 0.5) * noiseStrength;

                // Áp dụng nhiễu Speckle cho từng kênh màu
                int newR = qBound(0, static_cast<int>(r * speckleFactor), 255);
                int newG = qBound(0, static_cast<int>(g * speckleFactor), 255);
                int newB = qBound(0, static_cast<int>(b * speckleFactor), 255);

                image_out.setPixel(x, y, qRgb(newR, newG, newB));
            }
        }
